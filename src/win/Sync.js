...
function Sync {
    ...
    var STATUS_EXTRACTING = 2;
    var STATUS_COMPLETE = 3;

    var INVALID_URL_ERROR = 1;
    var CONNECTION_ERROR = 2;
    var UNZIP_ERROR = 3;

    var PROP_LOCAL_PATH = "localPath";
    var PROP_STATUS = "status";
    var PROP_PROGRESS = "progress";
    var PROP_LOADED = "loaded";
    var PROP_TOTAL = "total";
    var PROP_CACHED = "cached";
    // Type
    var TYPE_REPLACE = "replace";
    var TYPE_MERGE = "merge";
    var TYPE_LOCAL = "local";

    var LOG_TAG = "ContentSync";
    ...
    function ProgressEvent {
        ...
        functionfunction ProgressEvent() {
            this.status = STATUS_STOPPED;
        }
        functionfunction getLoaded() {
            return loaded;
        }
        functionfunction setLoaded(loaded) {
            this.loaded = loaded;
            updatePercentage();
        }
        functionfunction addLoaded(add) {
            this.loaded += add;
            updatePercentage();
        }
        functionfunction getTotal() {
            return total;
        }
        functionfunction setTotal(total) {
            this.total = total;
            updatePercentage();
        }
        functionfunction getStatus() {
            return status;
        }
        functionfunction setStatus(status) {
            this.status = status;
        }
        functionfunction isAborted() {
            return aborted;
        }
        function setAborted(aborted) {
        ...
        function toJSONObject() {
            var jsonProgress = {
                PROP_PROGRESS: this.percentage,
                PROP_STATUS: this.getStatus()
                PROP_LOADED: this.getLoaded()
                PROP_TOTAL: this.getTotal()
            };
            return JSON.stringify(jsonProgress);
        }
    ...
    function copyAssetFile(outputDirectory, filename, wwwExists) {
        var targetFile = filename;
        var startString = "www/";
        
        if (!wwwExists) {
            if(targetFile.slice(0, startString.length) == startString) {
                targetFile = targetFile.slice(startString.length, targetFile.length);
            }
        }

        var lastIndex = targetFile.lastIndexOf("/");
        if (lastIndex > 0) {
            // TO TRANSLATE
    ...
    function removeFolder(directory) {
        //if directory exists && directory is Directory
            //remove it
    }
}
